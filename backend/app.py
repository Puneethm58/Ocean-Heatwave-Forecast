...
def real_data():
    """
    Fetch latest global SST snapshot from NOAA AVHRR erdBAssta5day.
    Returns list of {lat, lon, sst} objects.
    """
    try:
    ...
    for row in reader:
        try:
            ...
            if -2 <= sst <= 35:           # physical SST bounds
                data.append({"lat": round(lat, 2), "lon": round(lon, 2), "sst": round(sst, 3)})
